...
class RouterosAPI
{
    ...
    public function connect($ip, $login, $password)
    {
        ...
        for ($ATTEMPT = 1; $ATTEMPT <= $this->attempts; $ATTEMPT++) {
            ...
            $this->debug('Connection attempt #' . $ATTEMPT . ' to ' . $PROTOCOL . $host . ':' . $this->port . '...');
            $this->socket = @stream_socket_client($PROTOCOL . $host.':'. $this->port, $this->error_no, $this->error_str, $this->timeout, STREAM_CLIENT_CONNECT,$context);
            if ($this->socket) {
